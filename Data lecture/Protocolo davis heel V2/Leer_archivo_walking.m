function [Data_walking Names] = Leer_archivo_walking(Pac) 


Pnum = num2str(Pac);
Nombre_Mar_Sta = ['Marcadores_Walking_P' Pnum '.txt'];
...
while ischar(oneline)
    ...
    if i > 11
        ...
        if length(Split_Char) > 5
            ...
            if IND > 1 & F_nan == 1 
                if length(Data_num) ~= length(Data_walking(IND-1,:));
                    diff = Data_walking(IND-1,:) - length(Data_num);
                    for x = length(Data_num) + 1:length(Data_walking(IND-1,:))
                        Data_num(x) = NaN;
                    end
                end
            end
